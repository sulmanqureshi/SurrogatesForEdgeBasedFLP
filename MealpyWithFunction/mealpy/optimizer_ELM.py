...
class Optimizer:
    ...
    def __init__(self, problem, kwargs=None):
        ...
        self.C = None
        self.hidden_units = 200  # ELM param
        self.activation_function = 'relu'  # ELM param
        self.random_type = 'normal'  # ELM param
        C = 1  # ELM param
    ...
    def evaluate_pop_mix_10_per(self, modelElm, pop):
    ...
    def solve(self, mode='single', starting_positions=None, n_workers=None):
        ...
        for epoch in range(0, self.epoch):
            ...
            if epoch <= 1:
                ...
                if epoch == 2:
                    func = lambda trial: self.objective(trial, df_pop.values, df_cost.values)
                    study = optuna.create_study(direction='minimize')
                    study.optimize(func, n_trials=100)
                    trial = study.best_trial
                    # print('Accuracy: {}'.format(trial.value))
                    # print("Best hyperparameters: {}".format(trial.params))
                    self.hidden_units = trial.params['hidden_units']
                    self.activation_function = trial.params['activation_function']
                    self.random_type = trial.params['random_type']
                    self.C = trial.params['C']

                modelElm = elm.elm(hidden_units=self.hidden_units, activation_function=self.activation_function,
                                   random_type=self.random_type, C=self.C, elm_type='reg', x=df_pop.values,
                                   y=df_cost.values)
                # modelElm = elm.elm(hidden_units=200, activation_function='sigmoid',
                #                    random_type='normal', C=1, elm_type='reg', x=df_pop.values,
                #                    y=df_cost.values)
                beta, train_score, running_time = modelElm.fit('no_re')
                cost_real, pop_real, cost, population, mse, accuracy_ = \
                    self.evaluate_pop_mix_10_per(modelElm, population)
                rmse.append(mse)
    ...
    def objective(self, trial, df_pop, df_cost):
        ...
        hidden_units = trial.suggest_int('hidden_units', 150, 250)
        activation_function = trial.suggest_categorical('activation_function',
                                                        ['sigmoid', 'relu', 'sin', 'tanh', 'leaky_relu'])
        random_type = trial.suggest_categorical('random_type', ['uniform', 'normal'])
        C = trial.suggest_int('C', 1, 3)

        modelElm = elm.elm(hidden_units=hidden_units, activation_function=activation_function, random_type=random_type,
                           C=C,
                           elm_type='reg', x=df_pop, y=df_cost)
        beta, train_score, running_time = modelElm.fit('no_re')
